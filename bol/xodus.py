...
script with the executable's NT path as its only argument. Standing here, in
between, is what lets the launcher keep its own launch command: this appends
the NT path to it and execs it, with the descriptor still open.
"""
import json
import os
import sys
...
nt_name = sys.argv[1] if len(sys.argv) > 1 else None
if not nt_name:
...
# The memfd is the only place it can land now, and the loader reads the field
# when it maps the image, so it has to happen before the exec below. Never
# fatal: a game that starts with a 1 MB stack still beats one that does not
# start at all.
try:
    ...
    for entry in (os.environ.get("WINE_DLL_FILE_MAP") or "").split("|"):
        ...
        if mapped != nt_name or not fd_text.isdigit():
            continue
        _raise_stack_reserve(int(fd_text))
        break
...
def wrap_encrypted_launch(argv, game_dir: Path, work_dir: Path,
    ...
    wrapper.write_text(
        ...
        encoding="utf-8")
    wrapper.chmod(0o755)
    # Name the executable explicitly. Xodus otherwise takes whichever
    # keep_encrypted entry its hash map yields first, which is not stable
    # across runs once a package encrypts more than one file.
    exe_key = "\\" + Path(argv[-1]).name
    return [str(binary), "run", str(game_dir), str(wrapper), "--exe", exe_key]
